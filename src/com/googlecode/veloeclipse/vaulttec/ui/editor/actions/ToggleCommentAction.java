...
public final class ToggleCommentAction extends TextEditorAction
{
    ...
    private boolean isSelectionCommented(ISelection selection)
    {
        ...
        try
        {
            ...
            for (int i = 0, j = 0; i < regions.length; i++, j += 2)
            {
                ...
                if (prefixes != null && prefixes.length > 0 && lines[j] >= 0 && lines[j + 1] >= 0) if (!isBlockCommented(lines[j], lines[j + 1], prefixes, document)) return false;
            }
            return true;
        }
    ...
    public void update()
    {
        ...
        ITextEditor editor = getTextEditor();
        if (fOperationTarget == null && editor != null) fOperationTarget = (ITextOperationTarget) editor.getAdapter(ITextOperationTarget.class);
